...
class Master extends DBConnection
{
	...
	function save_cargo()
	{
		...
		// $_POST['total'] = implode("|",$_POST['total']);
		// $_POST['cargo_type_id'] = implode("|",$_POST['cargo_type_id']);
		// $_POST['weight'] = implode("|",$_POST['weight']);
		//var_dump($_POST);
		...
		if (empty($id)) {
			...
			$sql = "UPDATE cargo_list set status = {$data} where id = '{$id}' ";
		}
		$save = $this->conn->query($sql);
		if ($save) {
			...
			$data = "";
			// var_dump($cargo_type_id);
			foreach ($_POST['cargo_type_id'] as $k => $v) {
				if (empty(trim($v)))
					continue;
				if (!empty($data)) $data .= ",";
				$data .= "('{$cid}', '{$this->conn->real_escape_string($v)}', '{$this->conn->real_escape_string($_POST['price'][$k])}', '{$this->conn->real_escape_string($_POST['weight'][$k])}', '{$this->conn->real_escape_string($_POST['total'][$k])}')";
			}
			if (!empty($data)) {
